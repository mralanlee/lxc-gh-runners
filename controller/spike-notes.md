...
3. **proxmoxer 2.3.0 source confirms QEMU-only.** The library contains explicit special-casing for `agent/exec` in two places, both scoped to QEMU:
   - `proxmoxer/backends/https.py:219` — `if k == "command" and url.endswith("agent/exec"):`
   - `proxmoxer/backends/command_base.py:69` — `if "/agent/exec" in url:`
   ...
   Proxmoxer wheel inspected: `/tmp/proxmoxer-src/proxmoxer-2.3.0-py3-none-any.whl`
...
def exec(vmid: int, cmd: list[str]) -> tuple[str, str, int]:
    ...
    Environment requirements:
      - PROXMOX_HOST env var (or config): hostname/IP of the Proxmox node.
    ...
    proxmox_host = _get_proxmox_host()   # read from env/config
...
- **`StrictHostKeyChecking=accept-new`** avoids first-connect prompt in automation while still protecting against MITM on subsequent calls. For stricter security, pre-populate the host key.
